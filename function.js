...
if (/android/i.test(navigator.userAgent) && window.matchMedia && window.matchMedia('(prefers-color-scheme: light)').matches) {
...
function injectCSSIntoTypebotShadow() {
    ...
    if (typebotElement && typebotElement.shadowRoot) {
        ...
        style.innerHTML = `
        * {margin: 0;padding: 0;}
        .hero {width: 100%;height: 100vh;background: #a9a9a9;display: flex;align-items: center;justify-content: center;}
        .music {width: 90%;max-width: 900px;padding: 30px;box-shadow: 0 0 20px rgb(0,0,0, 0.2);background-color: #fff;}
        .track {display: flex;align-items: center;margin: 10px 10px 20px 10px;position: relative;width: 260px;}
        .track .controls {width: 20px;margin-right: 10px;margin-top: 6px;cursor: pointer;}
        .track .controls button {background: transparent;border: 0;cursor: pointer;}
        .track .controls button svg {display: none;pointer-events: none;}
        .track .controls button[data-state="play"] svg.pause {display: block;}
        .track .controls button[data-state="pause"] svg.play {display: block;}
        .track .controls svg {fill: #a9a9a9;}
        .track .timeline {flex: 1;width: 100%;margin-left: 10px;margin-right: 10px;}
        .track .icon {position: relative;margin-left: 10px;}
        .track .icon svg {position: absolute;bottom: 0;left: -15px;pointer-events: none;user-select: none;}
        .track .avatar {width: 50px;height: 50px;object-fit: cover;border-radius: 50%;}
        .tempo {position: absolute;float: left;left: 42px;top: 45px;font-size: 13px;z-index: 995;color: grey;animation-name: mostrar;animation-duration: 4s;}
        `;
