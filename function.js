...
const allResposta = botBody.querySelector('.typebot-chat-view').querySelectorAll('.typebot-guest-bubble');  
      ...
      if(allResposta.length > respostaLength) {
          for (let i = respostaLength; i < allResposta.length; i++) {
            const iconContainer = document.createElement('div');
            now = new Date
            const checkIcon = '<span class="hora2">'+ (now.getHours()<10?'0':'') + now.getHours() +':'+ (now.getMinutes()<10?'0':'') + now.getMinutes() +'</span><svg id="checkIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 38.626 24.684" style="position: absolute;bottom: 2px;right: 5px;" height="20" width="18"><g id="Grupo_1" data-name="Grupo 1" transform="translate(-708.9 -601.383)"><path id="Caminho_6" data-name="Caminho 6" d="M728.035,623.468l1.382,1.482,17.929-20.334" transform="translate(-1.937 -1.117)" fill="none" stroke="#29afdf" stroke-linecap="round" stroke-width="3"></path><path id="Caminho_7" data-name="Caminho 7" d="M712.017,616.07l7.088,8.039,17.757-20.14" transform="translate(-1 -0.469)" fill="none" stroke="#29afdf" stroke-linecap="round" stroke-width="3"></path></g></svg>';
            iconContainer.innerHTML = checkIcon;
...
function injectCSSIntoTypebotShadow() {
    ...
    if (typebotElement && typebotElement.shadowRoot) {
        ...
        * {
        ...
        console.error('Elemento typebot-standard ou shadowRoot não encontrado.');
    }
}

injectCSSIntoTypebotShadow();

// Função para observar a adição de novos elementos <audio> no Shadow DOM e customizar existentes
function observeShadowDOMForAudio() {
    const typebotElement = document.querySelector('typebot-standard');

    if (typebotElement && typebotElement.shadowRoot) {
        console.log('Shadow DOM detectado. Iniciando observação dentro do Shadow DOM.');
        ...
        if (existingAudios.length > 0) {
            existingAudios.forEach(audio => {
                console.log('Áudio já existente encontrado no Shadow DOM:', audio);
                createPlayer(audio, shadowRoot);
            });
        } else {
            console.log('Nenhum áudio existente encontrado no Shadow DOM.');
        }
                ...
                if (mutation.type === 'childList') {
                    mutation.addedNodes.forEach(node => {
                        if (node.tagName === 'AUDIO') {
                            console.log('Novo elemento de áudio detectado no Shadow DOM:', node);
                            createPlayer(node, shadowRoot);
                        }
                    });
                } else if (mutation.type === 'attributes' || mutation.type === 'characterData') {
                    if (mutation.target.tagName === 'AUDIO') {
                        console.log('Modificação detectada em elemento de áudio no Shadow DOM:', mutation.target);
        ...
        console.log('Observando adições e modificações de elementos <audio> no Shadow DOM...');
    } else {
        console.log('Elemento <typebot-standard> ou Shadow DOM não encontrado.');
    }
}

// Função que customiza o player usando o WaveSurfer
function createPlayer(audioElement, shadowRoot) {
    ...
    container.innerHTML = '<div class="track"><div class="controls"><button id="'+controlId+'" data-state="play"><svg viewBox="0 0 34 34" height="34" width="34" preserveAspectRatio="xMidYMid meet" class="play" version="1.1" x="0px" y="0px" enable-background="new 0 0 34 34" xml:space="preserve"><path fill="#858a8d" d="M8.5,8.7c0-1.7,1.2-2.4,2.6-1.5l14.4,8.3c1.4,0.8,1.4,2.2,0,3l-14.4,8.3 c-1.4,0.8-2.6,0.2-2.6-1.5V8.7z"></path></svg><svg viewBox="0 0 34 34" height="34" width="34" preserveAspectRatio="xMidYMid meet" class="pause" version="1.1" x="0px" y="0px" enable-background="new 0 0 34 34" xml:space="preserve"><path fill="#858a8d" d="M9.2,25c0,0.5,0.4,1,0.9,1h3.6c0.5,0,0.9-0.4,0.9-1V9c0-0.5-0.4-0.9-0.9-0.9h-3.6 C9.7,8,9.2,8.4,9.2,9V25z M20.2,8c-0.5,0-1,0.4-1,0.9V25c0,0.5,0.4,1,1,1h3.6c0.5,0,1-0.4,1-1V9c0-0.5-0.4-0.9-1-0.9 C23.8,8,20.2,8,20.2,8z"></path></svg><div class="tempo"><span id="'+timeId+'" style="display:none;">0:00</span><span id="'+durationId+'">0:00</span></div></button></div><div class="timeline" id="'+waveformId+'"></div><div class="icon"><svg width="30px" height="30px" viewBox="0 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve"><g><path fill="#48d44f" d="M75.46,49.16h0.003v-0.098c0-0.006,0-0.013,0-0.019h0V34.787c0-2.088-1.693-3.781-3.782-3.781c-2.088,0-3.783,1.693-3.783,3.781v14.256c0,9.885-8.013,17.897-17.898,17.897s-17.898-8.013-17.898-17.897h0V34.787c0-2.088-1.693-3.781-3.782-3.781c-2.088,0-3.783,1.693-3.783,3.781V49.16h0.003c0.058,12.724,9.447,23.243,21.678,25.065v5.438H32.839v0.003c-2.074,0.018-3.75,1.701-3.75,3.779c0,2.078,1.676,3.761,3.75,3.779v0.003h34.29l0,0c2.089,0,3.782-1.693,3.782-3.782c0-2.089-1.693-3.782-3.782-3.782l0,0H53.782v-5.438C66.013,72.403,75.403,61.884,75.46,49.16z"></path><path fill="#48d44f" d="M37.186,48.941c0,7.088,5.745,12.833,12.833,12.833c7.087,0,12.831-5.746,12.831-12.833c0-0.096-0.012-0.188-0.014-0.283h0.053V25.322h-0.053c-0.153-6.955-5.826-12.549-12.817-12.549c-6.992,0-12.666,5.594-12.819,12.549h-0.052v23.336h0.052C37.199,48.753,37.186,48.845,37.186,48.941z"></path></g></svg><img class="avatar" src="'+var_avatar+'" alt="'+var_nome+'"></div></div>';

    let hostBubble = audioElement.closest('.typebot-host-bubble');
    if (hostBubble) {
        hostBubble.insertBefore(container, hostBubble.firstChild);
    } else {
        console.error('Elemento .typebot-host-bubble correspondente ao áudio não encontrado.');
    }

    const playPauseBtn = container.querySelector('#'+controlId);
    if (!playPauseBtn) {
        console.error('Botão de controle não encontrado no Shadow DOM.');
        return;
    }
    ...
    const style = document.createElement('style');
    style.innerHTML = '#'+waveformId+' ::part(cursor) {background-color: #25d366;width: 15px;height: 15px;border-radius: 50%;top: 50%;transform: translateY(-50%);}.track::after {position: absolute;content: "";width: 0;height: 0;border-style: solid;border-width: 0px 10px 10px 0;border-color: transparent #fff transparent transparent;top: 0;left: -10px;display: none;}';
...
loadWaveSurferScript(() => {
    console.log('WaveSurfer script carregado. Iniciando observação de áudio no Shadow DOM...');
